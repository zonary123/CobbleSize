...
public class ScalePokemonData {
  ...
  public ScalePokemonData(String pokemon, String form) {
    ...
    this.form = form;
    this.sizes = SizeChanceWithoutItem.transform(CobbleSize.config.getPokemonsizes());
  }
  ...
  public static ScalePokemonData getScalePokemonData(Pokemon pokemon) {
    ...
    List<ScalePokemonData> specifiedSizes = CobbleSize.config.getSpecifiedSizes();
  ...
  private static ScalePokemonData transformDefaultSizes() {
    if (!CobbleSize.config.isRandomsize()) return new ScalePokemonData("default", "default",
      List.of(
        new SizeChanceWithoutItem("Normal", 1, 100)
      ));
    return new ScalePokemonData("default", "default", SizeChanceWithoutItem.transform(CobbleSize.config.getPokemonsizes()));
  }
  ...
  public SizeChanceWithoutItem getRandomPokemonSize() {
    if (sizes == null || sizes.isEmpty() || !CobbleSize.config.isRandomsize()) return new SizeChanceWithoutItem();
